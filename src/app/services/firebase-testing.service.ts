...
export class FirebaseTestingService {
  ...
  private url: string =
    'https://angular-firebase-testing-9aa79-default-rtdb.asia-southeast1.firebasedatabase.app';
  ...
  postData(title: string, description: string) {
    return this.http.post(`${this.url}/posts.json`, {
      title: title,
  ...
  fetchData() {
    return this.http
      .get<{ [key: string]: FirebasePost }>(`${this.url}/posts.json`)
      .pipe(
  ...
  // This function deletes a post from firebase
  deletePost(id: string) {
    return this.http.delete(`${this.url}/posts/${id}.json`);
  }
}
